...
addForm.addEventListener("submit", async (e) => {
  ...
  if (addForm.checkValidity() === false) {
    ...
    const data = await fetch("../assets/php/crud_user.php", {
      method: "POST",
      body: formData,
    });
...
const fetchAllUsers = async () => {
  const data = await fetch("../assets/php/crud_user.php?read=1", {
    method: "GET",
  });
  const response = await data.text();
...
const deleteUser = async (id) => {
  const data = await fetch(`../assets/php/crud_user.php?delete=1&id=${id}`, {
    method: "GET",
  });
  const response = await data.text();
  showAlert.innerHTML = response;
